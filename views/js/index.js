...
// 初始化用户、密码输入框状态
document.getElementById('lgUsername').value = window.localStorage.getItem('name') || '';
...
lgBtn.addEventListener('click', function () {
    ...
    //获取用户名、密码
    let name = document.getElementById('lgUsername').value.trim();
    ...
    ajaxPost("/login", data).then(res => {
        ...
        if (code == 1) {
            ...
            let timer = setTimeout(function () {
                window.location.href = './pages/index.html?nickname=' + data.nickname;
                clearTimeout(timer);
            }, 1000);
